...
export default function TeacherLeave() {
  ...
  const load = () => {
    setLoading(true);
    api.get('/teacher-portal/leave/my')
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.post('/teacher-portal/leave/apply', { ...form, total_days: calcDays(form.from_date, form.to_date) });
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm">Apply for leave and track your applications</p>
          </div>
        </div>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              <div>
                ...
                <select required value={form.leave_type} onChange={e => setForm(p => ({ ...p, leave_type: e.target.value }))}
                  className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400 bg-white capitalize">
              ...
              <div>
                ...
                <textarea required value={form.reason} onChange={e => setForm(p => ({ ...p, reason: e.target.value }))} rows={3}
                  className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400 resize-none" placeholder="Explain your reason..." />
              </div>
