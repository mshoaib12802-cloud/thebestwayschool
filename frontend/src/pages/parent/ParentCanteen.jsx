...
import {
  UtensilsCrossed, CheckCircle2, AlertCircle, Clock,
  TrendingUp, ChevronDown, ChevronUp, Users,
} from 'lucide-react';
      ...
      <button className="w-full flex items-center justify-between px-5 py-4 hover:bg-slate-50/60 transition-colors" onClick={() => setOpen(o => !o)}>
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-teal-50 flex items-center justify-center">
            <UtensilsCrossed size={20} className="text-teal-500"/>
          </div>
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <p className="font-extrabold text-teal-700 text-lg">Rs. {charge.amount?.toLocaleString()}</p>
...
export default function ParentCanteen() {
  ...
  useEffect(() => {
    ...
    setLoadingCh(true);
    api.get(`/parent-portal/children/${selectedChild}/canteen`)
  ...
  if (loadingC) return <div className="flex items-center justify-center h-64"><div className="w-10 h-10 border-4 border-teal-500 border-t-transparent rounded-full animate-spin"/></div>;

  return (
    <div>
      <div className="mb-6">
        <h1 className="text-xl font-extrabold text-slate-800 flex items-center gap-2">
          <UtensilsCrossed size={22} className="text-teal-600"/> Canteen
        </h1>
        <p className="text-sm text-slate-500 mt-0.5">Monthly canteen charges for your child</p>
      </div>
        ...
        <div className="flex gap-2 mb-5 flex-wrap">
          {children.map(c => (
            <button key={c._id} onClick={() => setSelected(c._id)}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold border transition-all ${selectedChild === c._id ? 'bg-teal-600 text-white border-teal-600' : 'bg-white text-slate-600 border-slate-200 hover:border-teal-400'}`}>
              <Users size={14}/> {c.full_name}
            </button>
          ))}
        </div>
      )}
      ...
      {loadingCh ? (
        <div className="flex items-center justify-center h-48"><div className="w-8 h-8 border-4 border-teal-500 border-t-transparent rounded-full animate-spin"/></div>
      ) : charges.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center justify-center py-20 text-slate-400">
          ...
          <p className="text-sm mt-1">Canteen bills will appear here once set by the school.</p>
        </div>
      ) : (
        <>
          {/* Summary */}
          <div className="grid grid-cols-3 gap-3 mb-6">
            {[
              { label: 'Total Billed', value: `Rs. ${total.toLocaleString()}`,   color: 'bg-teal-50 text-teal-700',   icon: <TrendingUp size={18}/> },
